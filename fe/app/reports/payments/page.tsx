...
const PAYMENT_REPORT_PRINT_STYLE_ID = "payment-report-print-style";

function installPaymentReportPrintStyle() {
  if (document.getElementById(PAYMENT_REPORT_PRINT_STYLE_ID)) return;

  const style = document.createElement("style");
  style.id = PAYMENT_REPORT_PRINT_STYLE_ID;
  style.textContent = `
    @media print {
      @page {
        size: A4 landscape;
...
function removePaymentReportPrintStyle() {
  document.getElementById(PAYMENT_REPORT_PRINT_STYLE_ID)?.remove();
}
...
export default function PeriodicPaymentReportPage() {
  ...
  useEffect(() => {
    document.body.classList.add("payment-report-print-page");
    window.addEventListener("beforeprint", installPaymentReportPrintStyle);
    window.addEventListener("afterprint", removePaymentReportPrintStyle);

    return () => {
      document.body.classList.remove("payment-report-print-page");
      window.removeEventListener("beforeprint", installPaymentReportPrintStyle);
      window.removeEventListener("afterprint", removePaymentReportPrintStyle);
      removePaymentReportPrintStyle();
    };
  }, []);
  ...
  return (
    <div className="min-h-screen bg-background relative">
      <AbstractBg />
      <Header />
      <main className="payment-report-page relative p-6 max-w-5xl mx-auto print:p-0 print:max-w-none">
              ...
              {report && (
                <>
                  ...
                  <Button
                    variant="outline"
                    onClick={() => {
                      installPaymentReportPrintStyle();
                      window.print();
                    }}
                    className="gap-1.5"
                  >
              ...
              <CardContent className="print:p-0">
                {report.payments.length === 0 ? (
                  ...
                  <div className="rounded-lg border overflow-x-auto print:overflow-visible">
                    <table className="payment-report-table w-full text-sm min-w-[800px] print:min-w-0">
                      <colgroup>
                        <col className="payment-report-col-date" />
                        <col className="payment-report-col-member" />
                        <col className="payment-report-col-amount" />
                        <col className="payment-report-col-method" />
                        <col className="payment-report-col-receipt" />
                        <col className="payment-report-col-collector" />
                        <col className="payment-report-col-status" />
                        <col className="payment-report-col-reversal" />
                      </colgroup>
                      <thead className="bg-muted/50">
                        <tr className="hidden print:table-row">
                          <th colSpan={8} className="text-left p-2.5 font-semibold bg-background">
                            Payment Details
                          </th>
                        </tr>
                        <tr>
