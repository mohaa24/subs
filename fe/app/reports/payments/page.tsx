    ...
    <div className="min-h-screen bg-slate-50/60">
      ...
      <main className="mx-auto max-w-[1800px] p-3 sm:p-4 md:p-6 print:max-w-none print:p-0">
        <div className="print:hidden">
          <Breadcrumb items={[{ label: "Dashboard", href: dashboardFlowHref("reports") }, { label: "Financial Reports", href: "/finance-reports" }, { label: "Member Payment Report" }]} />
...
function Breakdown({ title, description, items }: { title:string; description:string; items:Array<{ label:string; amount:number }> }) {
  return <div className="payment-breakdown grid gap-4 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 sm:px-5 md:grid-cols-[300px_1fr] print:grid-cols-[190px_1fr]">
    <div className="border-b border-slate-200 pb-3 md:border-b-0 md:border-r md:pb-0 md:pr-5 print:border-b-0 print:border-r print:pb-0 print:pr-4"><h3 className="text-sm font-bold text-slate-900">{title}</h3><p className="mt-0.5 text-xs text-slate-500">{description}</p></div>
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:auto-cols-fr lg:grid-flow-col lg:grid-cols-none print:auto-cols-fr print:grid-flow-col print:grid-cols-none">{items.length ? items.map((item) => <div key={item.label} className="min-w-0 border-r border-slate-200 pr-3 last:border-0"><p className="break-words text-[10px] font-bold uppercase tracking-wide text-slate-500">{item.label}</p><p className={`mt-0.5 text-sm font-bold tabular-nums ${item.amount < 0 ? "text-red-600" : "text-slate-900"}`}>{amount(item.amount)}</p></div>) : <p className="col-span-2 text-xs text-slate-500">No collections in this period.</p>}</div>
  </div>;
}
...
function MovementCards({ movements }: { movements:PaymentMovement[] }) {
  if (!movements.length) return <div className="mt-4 rounded-xl border border-slate-200 p-8 text-center text-sm text-slate-500 md:hidden print:hidden">No payments found for the selected period.</div>;
  return <div className="mt-4 space-y-3 md:hidden print:hidden">{movements.map((movement) => <article key={movement.key} className={`overflow-hidden rounded-xl border ${movement.status === "posted" ? "border-slate-200 bg-white" : "border-red-200 bg-red-50/50"}`}>
    <div className="flex items-start justify-between gap-3 border-b border-inherit px-4 py-3">
      <div className="min-w-0"><p className="font-semibold text-slate-900">{reportDate(movement.movementDate)}</p><p className="mt-0.5 text-[11px] text-slate-500">Entered {reportDate(movement.enteredAt)}</p></div>
      <StatusBadge status={movement.status} />
    </div>
    <div className="space-y-3 px-4 py-3 text-sm">
      <div className="flex items-start justify-between gap-4"><div className="min-w-0"><p className="font-semibold text-slate-900">{movement.memberName}</p><p className="mt-0.5 text-xs text-slate-500">ID {movement.membershipId} · {movement.zone}</p></div><div className="shrink-0 text-right"><p className={`text-base font-bold tabular-nums ${movement.amount < 0 ? "text-red-600" : "text-slate-900"}`}>{amount(movement.amount)}</p><p className="mt-0.5 text-[10px] uppercase tracking-wide text-slate-500">Amount</p></div></div>
      <div className="rounded-lg bg-slate-50 px-3 py-2"><p className="text-[10px] font-bold uppercase tracking-wide text-slate-500">Receipt / Link</p><p className="mt-0.5 break-all font-semibold text-slate-900">{movement.receiptNumber}</p>{movement.linkedReceiptNumber && <p className="mt-0.5 break-all text-xs font-medium text-red-600">{movement.type === "payment" ? "Linked reversal" : "Reversal of"}: {movement.linkedReceiptNumber}</p>}</div>
      <div className="grid grid-cols-2 gap-3"><div><p className="text-[10px] font-bold uppercase tracking-wide text-slate-500">Payment Method</p><p className="mt-0.5 text-slate-700">{movement.paymentMethod}</p></div><div><p className="text-[10px] font-bold uppercase tracking-wide text-slate-500">Running Total</p><p className="mt-0.5 font-semibold tabular-nums text-slate-900">{amount(movement.runningTotal)}</p></div></div>
      <div><p className="text-[10px] font-bold uppercase tracking-wide text-slate-500">Entered By</p><p className="mt-0.5 break-all text-slate-700">{movement.enteredBy}</p></div>
      {movement.reversalReason && <div className="rounded-lg border border-red-100 bg-red-50 px-3 py-2"><p className="font-semibold text-red-600">{movement.reversalReason}</p>{movement.relatedDate && <p className="mt-0.5 text-xs text-slate-500">{movement.type === "payment" ? "Reversed" : "Original entry"} {reportDate(movement.relatedDate)}</p>}</div>}
    </div>
  </article>)}</div>;
}

function Reconciliation({ report }: { report:MemberPaymentReport }) {
  return <div className="payment-reconciliation mt-5 flex flex-col gap-4 rounded-xl border border-slate-200 bg-slate-50 px-4 py-4 sm:px-5 md:flex-row md:items-center md:justify-between print:flex-row print:items-center print:justify-between"><div><h3 className="text-xs font-bold uppercase tracking-wide text-slate-600">Payment Reconciliation</h3><div className="mt-2 grid gap-2 text-sm sm:flex sm:flex-wrap sm:items-baseline sm:gap-x-5 sm:gap-y-1"><span className="flex justify-between gap-3 sm:block">Total Received <strong className="tabular-nums sm:ml-2">{money(report.totalCollected)}</strong></span><span className="hidden sm:inline">-</span><span className="flex justify-between gap-3 sm:block">Reversals <strong className="tabular-nums text-red-600 sm:ml-2">{money(report.totalReversed)}</strong></span><span className="hidden sm:inline">=</span><span className="flex justify-between gap-3 border-t border-slate-200 pt-2 font-semibold sm:block sm:border-0 sm:pt-0">Net Received <strong className="tabular-nums text-emerald-700 sm:ml-2">{money(report.netCollected)}</strong></span></div></div><p className="max-w-md text-left text-xs leading-5 text-slate-500 md:text-right print:text-right">Posted payments increase the running total. Reversal lines reduce it and remain linked to the original receipt.</p></div>;
}

function ReportFooter({ page, totalPages }: { page:number; totalPages:number }) {
  return <div className="payment-report-footer mt-5 flex flex-wrap items-center justify-between gap-2 border-t border-slate-200 pt-3 text-[11px] text-slate-500"><span>Member Payment Report</span><span className="flex items-center gap-1"><FileText className="h-3 w-3" />Powered by Civica</span><span>Page {page} of {totalPages}</span></div>;
}
...
function SummaryTile({ icon, label, value, detail, tone }: { icon:ReactNode; label:string; value:string; detail?:string; tone:"received"|"reversed"|"net"|"movements" }) {
  ...
  return <div className={`payment-summary-tile min-h-[112px] rounded-xl border p-4 sm:min-h-[126px] sm:p-5 ${styles}`}><div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wide sm:text-sm"><span className="[&>svg]:h-5 [&>svg]:w-5">{icon}</span>{label}</div><p className="summary-value mt-3 text-[24px] font-bold leading-none tabular-nums sm:text-[28px]">{value}</p>{detail && <p className="mt-2 text-xs opacity-75">{detail}</p>}</div>;
}
