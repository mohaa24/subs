...
import { ArrowLeftRight, Banknote, BanknoteArrowDown, BanknoteArrowUp, BetweenHorizontalStart, Calendar1, CalendarDays, ChevronRight, Info, Plus, ReceiptText, Search, WalletCards } from "lucide-react";
      ...
      <main className="relative z-10 mx-auto max-w-7xl space-y-6 p-6">
        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
          ...
          <div className="flex flex-wrap items-start gap-2">
            ...
            {kind === "payable" ? (
              <Button asChild variant="addNew" className="mt-5">
                <Link href="/accounting?tab=accounts&accountType=liability&assetSubtype=loan_payable"><Plus className="mr-2 h-4 w-4" />New Borrowing</Link>
              </Button>
            ) : null}
          </div>
        </div>
        ...
        {kind === "payable" ? (
          <div className="grid gap-3 md:grid-cols-4">
            ...
            <Metric intent="cashIn" icon={BanknoteArrowDown} label="Total Borrowed" value={formatRs(totals?.totalBorrowed ?? 0)} helper="Total amount added across all accounts" />
            <Metric intent="cashOut" icon={BanknoteArrowUp} label="Total Settled" value={formatRs(totals?.totalRepaid ?? 0)} helper="Total amount paid across all accounts" />
          ...
          <div className="grid gap-3 md:grid-cols-3">
            <Metric intent="cashIn" icon={BanknoteArrowDown} label="YTD Income" value={formatRs(overview?.totals.periodTotal ?? 0)} />
...
function Metric({ icon, label, value, helper, intent }: { icon: typeof ReceiptText; label: string; value: string; helper?: string; intent: MetricTileIntent }) {
  return <MetricTile icon={icon} label={label} value={value} helper={helper} intent={intent} />;
}
