...
import { MAX_SMS_SEGMENTS, smsSegmentInfo } from "@/lib/sms-segments";
...
type Estimate = {
  ...
  estimatedSmsCount: number;
  overLimitRecipients: Array<{ membershipId: string; membershipNo: string; memberName: string; estimatedSmsCount: number }>;
...
function SmsSegmentCounter({ value, personalized = false }: { value: string; personalized?: boolean }) {
  const info = smsSegmentInfo(value);
  const overLimit = info.segments > MAX_SMS_SEGMENTS;
  return (
    <div className={`mt-2 rounded-md border px-3 py-2 text-xs ${overLimit ? "border-red-200 bg-red-50 text-red-700" : "bg-muted/30 text-muted-foreground"}`}>
      <div className="flex flex-wrap items-center justify-between gap-2">
        <span>{info.encoding === "unicode" ? "Unicode (Sinhala/Tamil)" : "Plain text"} · {info.characters} characters</span>
        <strong className={overLimit ? "text-red-700" : "text-foreground"}>{info.segments} / {MAX_SMS_SEGMENTS} segments</strong>
      </div>
      <p className="mt-1">
        {info.remainingInSegment} characters remaining in this segment · {info.encoding === "unicode" ? "70 characters for one segment, then 67 per segment" : "155 characters per segment"}.
      </p>
      {personalized && <p className="mt-1">The final count is calculated separately after placeholders are filled for each recipient.</p>}
      {overLimit && <p className="mt-1 font-medium">Shorten the message to {info.maximumCharacters} characters or fewer for this encoding.</p>}
    </div>
  );
}
...
export function AnnouncementsWorkspace({ section = "announcements" }: { section?: "announcements" | "templates" | "groups" }) {
  ...
  return (
    <div className="min-h-screen bg-background relative">
        ...
        <DialogContent className="max-h-[94vh] max-w-4xl overflow-y-auto p-0">
          ...
          <div className="grid gap-5 p-5 lg:grid-cols-[1fr_280px]">
            <div className="space-y-5">
              ...
              <div><div className="mb-1.5 flex items-center justify-between"><label className="text-sm font-medium">Message</label><span className="text-xs text-muted-foreground">Maximum {MAX_SMS_SEGMENTS} segments per recipient</span></div><Textarea ref={messageRef} value={message} onChange={(event) => setMessage(event.target.value)} rows={7} placeholder="Write your announcement…" /><SmsSegmentCounter value={message} personalized /><div className="mt-2 flex flex-wrap gap-1.5">{VARIABLES.map((variable) => <button type="button" key={variable} onClick={() => insertVariable(variable, "message")} className="rounded border bg-muted/40 px-2 py-1 font-mono text-[11px] text-muted-foreground hover:border-primary hover:text-primary">{`{{${variable}}}`}</button>)}</div></div>
            </div>
            <aside className="space-y-3">
              ...
              <div className="rounded-xl border p-4"><p className="text-sm font-medium">Send estimate</p>{estimating ? <p className="mt-3 text-sm text-muted-foreground">Calculating…</p> : estimate ? <div className="mt-3 space-y-2 text-sm"><div className="flex justify-between"><span className="text-muted-foreground">Selected</span><span>{estimate.selectedCount}</span></div><div className="flex justify-between"><span className="text-muted-foreground">With phone</span><span>{estimate.eligibleCount}</span></div><div className="flex justify-between"><span className="text-muted-foreground">No phone</span><span className={estimate.missingPhone.length ? "text-amber-600" : ""}>{estimate.missingPhone.length}</span></div><div className="border-t pt-2"><div className="flex justify-between font-medium"><span>Will consume</span><span>{estimate.estimatedSmsCount} segments</span></div></div>{estimate.overLimitRecipients.length > 0 ? <p className="rounded-md bg-red-50 p-2 text-xs text-red-700">{estimate.overLimitRecipients.length} personalized message{estimate.overLimitRecipients.length === 1 ? "" : "s"} exceed the {MAX_SMS_SEGMENTS}-segment limit. Shorten the message before sending.</p> : !estimate.canSend ? <p className="rounded-md bg-red-50 p-2 text-xs text-red-700">The available organization quota is not enough for this announcement.</p> : null}</div> : <p className="mt-3 text-xs text-muted-foreground">Select recipients and enter a message to calculate quota usage.</p>}</div>
            </aside>
          </div>
      ...
      <Dialog open={templateOpen} onOpenChange={setTemplateOpen}><DialogContent className="max-w-xl"><DialogHeader><DialogTitle>{editingTemplate ? "Edit template" : "New template"}</DialogTitle></DialogHeader><div className="space-y-4"><div><label className="mb-1 block text-sm font-medium">Name</label><Input value={templateName} onChange={(event) => setTemplateName(event.target.value)} /></div><div><label className="mb-1 block text-sm font-medium">Description</label><Input value={templateDescription} onChange={(event) => setTemplateDescription(event.target.value)} /></div><div><label className="mb-1 block text-sm font-medium">Message</label><Textarea rows={7} value={templateBody} onChange={(event) => setTemplateBody(event.target.value)} /><SmsSegmentCounter value={templateBody} personalized /><div className="mt-2 flex flex-wrap gap-1.5">{VARIABLES.map((variable) => <button type="button" key={variable} onClick={() => insertVariable(variable, "template")} className="rounded border px-2 py-1 font-mono text-[11px] text-muted-foreground">{`{{${variable}}}`}</button>)}</div></div><div className="flex justify-end gap-2"><Button variant="outline" onClick={() => setTemplateOpen(false)}>Cancel</Button><Button onClick={saveTemplate} disabled={saving || !templateName.trim() || !templateBody.trim()}>{saving ? "Saving…" : "Save template"}</Button></div></div></DialogContent></Dialog>
