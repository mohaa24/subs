export const PERMISSION_CATALOG = [
  ...
  { category: "Member dues & payments", key: "SEND_MEMBER_MESSAGE", label: "Send receipt SMS", description: "Send payment receipt messages.", implies: ["VIEW_MEMBER_PAYMENTS"] },
  { category: "Member dues & payments", key: "SEND_SMS_REMINDER", label: "Send SMS Reminder", description: "Send a payment reminder from a member's Payment tab.", implies: ["VIEW_MEMBER_DUES"] },
...
export const PERMISSION_KEYS = new Set<string>(PERMISSION_CATALOG.map((item) => item.key));

export function expandPermissions(permissions: string[]) {
  const expanded = new Set(permissions.filter((permission) => PERMISSION_KEYS.has(permission)));
  let changed = true;
  while (changed) {
    changed = false;
    for (const item of PERMISSION_CATALOG) {
      ...
      for (const implied of "implies" in item ? item.implies : []) {
        if (!expanded.has(implied)) { expanded.add(implied); changed = true; }
      }
    }
  }
  return [...expanded];
