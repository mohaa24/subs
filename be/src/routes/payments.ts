...
function buildOrganizationPaymentHistoryWhere(options: {
  ...
  const and: Prisma.PaymentWhereInput[] = [nonSystemAdjustmentOrStandaloneCreditFilter()];
  ...
  const payment = await withReceiptNumberRetry(() =>
    prisma.$transaction(async (tx) => {
      ...
      const recipientPhone =
        messageMembership?.hod?.mobileNumber || messageMembership?.hod?.whatsAppNumber;
      if (messageMembership?.hod && recipientPhone) {
        await queuePaymentReceived(tx, {
          ...
          memberName:
            messageMembership.hod.nameWithInitials ||
...
paymentsRouter.get("/report/periodic", async (req, res) => {
  ...
  const paymentMethodTotals = new Map<string, Decimal>();
  for (const payment of receiptPayments) {
