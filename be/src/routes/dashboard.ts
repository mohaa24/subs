...
dashboardRouter.get("/", async (req, res) => {
  ...
  for (const payment of payments) {
    ...
    if (payment.isReversed && payment.reversedAt) {
      ...
      if (reversalBucket) reversalBucket.cashOut += amount;
    }
  }
  ...
  for (const transaction of cashTransactions) {
    ...
    if (transactionBucket && isInflow) transactionBucket.cashIn += amount;
    if (transactionBucket && isOutflow) transactionBucket.cashOut += amount;
    if (transaction.reversedAt) {
      const reversalBucket = bucketFor(transaction.reversedAt);
      if (reversalBucket && isInflow) reversalBucket.cashOut += amount;
      if (reversalBucket && isOutflow) reversalBucket.cashIn += amount;
    }
  }
  ...
  for (const transaction of fundTransactions) {
    const amount = Number(transaction.amount);
    const transactionBucket = bucketFor(transaction.transactionDate);
    if (transactionBucket && transaction.transactionType === "collection") transactionBucket.cashIn += amount;
    if (transactionBucket && transaction.transactionType === "expense") transactionBucket.cashOut += amount;
    if (transaction.reversedAt) {
      const reversalBucket = bucketFor(transaction.reversedAt);
      if (reversalBucket && transaction.transactionType === "collection") reversalBucket.cashOut += amount;
      if (reversalBucket && transaction.transactionType === "expense") reversalBucket.cashIn += amount;
    }
  }
